...
const deepDiveIcons = [Shuffle, ShieldCheck, Layers, ScanLine, Target, FlaskConical];
...
export default function PapierkramOrakelCaseStudy() {
  ...
  const shotsInView = useInView(shotsRef, { once: true, margin: "-80px" });
    ...
    >
      ...
      <section ref={archRef} className="py-20 px-6">
        <div className="max-w-5xl mx-auto">
          ...
          {/* Screenshots */}
          <div ref={shotsRef} className="mt-16">
            <motion.h3
              initial={mounted ? { opacity: 0, y: 20 } : false}
              animate={shotsInView ? { opacity: 1, y: 0 } : {}}
            ...
            <div className="space-y-8">
              {tx.screenshots.items.map((shot, i) => (
                <motion.figure
                  key={shot.src}
                  initial={mounted ? { opacity: 0, y: 20 } : false}
                  animate={shotsInView ? { opacity: 1, y: 0 } : {}}
                ...
                >
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={shot.src} alt={shot.caption} className="w-full h-auto block" />
                  <figcaption
                    className="text-xs leading-relaxed p-4 text-center"
                    style={{
                      color: "var(--text-3)",
                      background: "color-mix(in srgb, var(--bg-card) 50%, transparent)",
                    }}
